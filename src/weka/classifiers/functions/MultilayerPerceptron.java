...
/*      if (m_controlPanel != null) {
  ...
  /**
	 * Call this function to get the total number of weights in the network.
	 * @return Total number of weights in the network.
	 */
	public int getNumWeights() {
		return ((m_numAttributes + 1) * (m_neuralNodes.length - m_numClasses) +
				(m_numClasses * (m_neuralNodes.length - m_numClasses + 1)));
	}
	
	/*Disable using default model*/
	public void EnableDefaultModel(boolean enable)
	{
		m_useDefaultModel = enable;
	}
	
	/**
     * Set attribute ranges
     */
    public void setAttributeRanges(double[] range){
  	  m_attributeRanges = range;
    }
    
    /**
     * Get attribute ranges
     */
    public double[] getAttributeRanges(){
  	  return m_attributeRanges;
    }
    
    public int getNumOfAttributeRanges(){
    	return m_attributeRanges.length;
    }
    
    /**
     * Set Attribute Bases
     */
    public void setAttributeBases(double[] bases){
  	  m_attributeBases = bases;
    }
    
    /**
     * Get Attribute Bases
     */
    public double[] getAttributeBases(){
  	  return m_attributeBases;
    }
	
    public int getNumOfAttributeBases(){
    	return m_attributeBases.length;
    }
    
	/**
	 * Call this function to set the weights of the network.
	 */
    public void importWeights(double[] allWeights) {
		double[] weightsNodeBuffer = null;

		for (int i = 0; i < m_numClasses; i++) {
			weightsNodeBuffer = new double[((NeuralNode) m_neuralNodes[i])
					.getWeights().length];
			System.arraycopy(allWeights, i
					* (m_neuralNodes.length - m_numClasses + 1),
					weightsNodeBuffer, 0, m_neuralNodes.length - m_numClasses
							+ 1);
			for (int j = m_neuralNodes.length - m_numClasses + 1; j < weightsNodeBuffer.length; j++) {
				weightsNodeBuffer[j] = 0.0;
			}
			((NeuralNode) m_neuralNodes[i]).setWeights(weightsNodeBuffer);
		}

		for (int i = 0; i < m_neuralNodes.length - m_numClasses; i++) {
			weightsNodeBuffer = new double[((NeuralNode) m_neuralNodes[i
					+ m_numClasses]).getWeights().length];
			System.arraycopy(allWeights, m_numClasses
					* (m_neuralNodes.length - m_numClasses + 1) + i
					* (m_numAttributes + 1), weightsNodeBuffer, 0,
					m_numAttributes + 1);
			((NeuralNode) m_neuralNodes[i + m_numClasses])
					.setWeights(weightsNodeBuffer);
		}

	}
	
	/**
	 * Call this function to get the weights of the network.
	 * @return Double array of weights.
	 */
	public double[] exportWeights() {
		int weights_count = getNumWeights();
		double[] allWeights = new double[weights_count];
		double[] weightsNodeBuffer = new double[m_numAttributes+1];
		
		for (int i = 0; i < m_numClasses; i++) {
			weightsNodeBuffer = ((NeuralNode)m_neuralNodes[i]).getWeights();
			System.arraycopy(weightsNodeBuffer, 0, allWeights,
							 i * (m_neuralNodes.length - m_numClasses + 1),
							 m_neuralNodes.length - m_numClasses + 1);
		}
		
		for (int i = 0; i < m_neuralNodes.length - m_numClasses; i++) {
			weightsNodeBuffer = ((NeuralNode)m_neuralNodes[i + m_numClasses]).getWeights();
			System.arraycopy(weightsNodeBuffer, 0, allWeights,
							 m_numClasses * (m_neuralNodes.length - m_numClasses + 1) + i * (m_numAttributes + 1),
							 m_numAttributes + 1);
		}
...
/*    if (m_gui) {
   ...
   */
  public void setupInputs() throws Exception {
   ...
   */
  public void setupOutputs() throws Exception {
